(function () {
	 ...
	 * syntax tree of a JavaScript file using the Ravena View Language. It is built
	...
	function error(message) {
		console.error("[Ravena] त्रुटि: " + message);
	}
	/**
	...
	function generateName(nameTree) {
		...
		return names.indexOf(name) === -1 ? name : "Ravena.view.components." + name;
	}
	/**
	 * Generator
	 *
	 * The generator takes parse nodes and converts them to strings representing
	 * JavaScript code. All code is generated the same, but Ravena view expressions
	...
	function generate(tree) {
		...
		if (typeof tree === "string") {
			...
			return {
				output: textGeneratedIsWhitespace ? textGenerated : "Ravena.view.components.text({data:\"" + textGenerated.replace(textSpecialRE, function (match, character, characterSpecial) {
					return character + (characterSpecial === "\"" ? "\\\"" : "\\n\\\n");
				}) + "\"})",
				isWhitespace: textGeneratedIsWhitespace
			};
		} else if (type === "interpolation") {
			return "Ravena.view.components.text({data:" + generate(tree.value[1]) + "})";
		} else if (type === "node") {
